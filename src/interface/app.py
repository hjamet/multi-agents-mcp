...
# 1. Global Refresh (Single Page App)
...
with st.sidebar:
    ...
    # --- ADMIN: COCKPIT ---
    with st.expander("🎛️ COCKPIT"):
        preset_dir = os.path.join("assets", "presets")
        if not os.path.exists(preset_dir):
            os.makedirs(preset_dir, exist_ok=True)
            
        st.caption("💾 Presets")
        save_name = st.text_input("Save Name", placeholder="scenaro_1")
        ...
        selected_preset = st.selectbox("Load Preset", presets) if presets else None
        ...
        st.divider()
        st.caption("🌍 Context")
        global_context = st.text_area("Context", config.get("context", ""), height=100)
        ...
        for i, p in enumerate(profiles):
            c_label, c_ctrl = st.columns([2, 2])
            c_label.text(p['name'])
            count = int(p.get("count", 0))
            if c_ctrl.button(f"➖ {p['name']}", key=f"sb_d_{i}"):
                p["count"] = max(0, count - 1)
                save_config(config)
                st.rerun()
            c_ctrl.markdown(f"**{count}**")
            if c_ctrl.button(f"➕ {p['name']}", key=f"sb_i_{i}"):
                p["count"] = count + 1
                save_config(config)
                st.rerun()
    ...
    with st.expander("🛠️ EDITOR"):
        ...
        selected_name = st.selectbox("Profile", profile_names, index=sel_idx, key="sb_edit_sel")
        ...
        if current_profile:
            st.markdown("---")
            # Vertical Layout
            new_name = st.text_input("Name", current_profile.get("name", ""))
            disp = st.text_input("Display", current_profile.get("display_name", ""))
            new_desc = st.text_input("Desc", current_profile.get("description", ""))
            new_prompt = st.text_area("Prompt", current_profile.get("system_prompt", ""), height=100)
            
            st.caption("Capabilities")
            caps = current_profile.get("capabilities", [])
            has_pub = st.checkbox("Public", "public" in caps)
            has_priv = st.checkbox("Private", "private" in caps)
            has_aud = st.checkbox("Audience", "audience" in caps)
            has_open = st.checkbox("Open Mode", "open" in caps)
